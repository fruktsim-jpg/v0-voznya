...
interface PlayerCardProps {
  ...
  /** Phase B — «Тёмный друн о тебе» (server, read-only). Прокидывается слотом,
   *  как и другие server-блоки; null/пусто → ничего не рендерится. */
  drunSlot?: ReactNode
}
...
export function PlayerCard({
  ...
  return (
    ...
    <div className="mx-auto max-w-4xl px-4 pb-10 sm:px-6">
      ...
      {/* ============================================================== */}
      {/* Phase B — «Тёмный друн о тебе»: мнение/связи/память (read-only).*/}
      {/* Server-слот: данные из ai_profiles + ai_memories, fail-silent.  */}
      {/* Идёт после «как он играет», перед коллекцией — это «кто он» для  */}
      {/* друна, личностный слой над сухой статой.                        */}
      {/* ============================================================== */}
      {drunSlot}
